        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a
                        href="{{ route('category', [$product->category[0]->slug]) }}">{{ $product->category[0]->category_name }}</a>
                </li>
                ...
                <div class="product-previw">
                    ...
                    <div class="products-images-nav">
                        ...
                        file_exists(public_path('images/'.$product_image->product_variation_image)))
                            <figure>
                                <img class="product-preview-list"
                                    src="{{ asset('images/'.$product_image->product_variation_image) }}"
                                    alt="{{ $product->product_name }}">
                            </figure>
                        @endif
                        @endforeach
                ...
                <div class="products-details">
                    ...
                    <div class="action-buttons py-4">
                        ...
                        <div class="sizes mt-3">
                            ...
                            <button id="productsizes{{$loop->iteration}}"
                                class="product-sizes-listing {{ ($loop->iteration == 1) ? 'active' : '' }}"
                                data-size="{{ $product_size->size }}" data-sp="{{ $product_price }}"
                ...
                <div class="product-card"  data-pcount="{{ $loop->iteration }}" data-section="numberRelateableProducts">
                        ...
                        <a href="{{ route('main_product', [$related_product->slug]) }}">
                            <figure>
                                <img class="d-block w-100"
                                    src="{{ ($related_product->product_image != '') && file_exists(public_path('images/'.$related_product->product_image)) ? asset('images/'.$related_product->product_image) : asset('images/default.png') }}"
                        ...
                        <div class="hover-content">
                            <ul>
                                ...
                                <li class="View-details"><a data-bs-toggle="tooltip" data-bs-placement="left"
                                        title="View details"
                                        href="{{ route('main_product', [$related_product->slug]) }}"><i
                    ...
                    <div class="card-body">
                        ...
                        <div class="add-to-cart-button">
                            <form>
                                <div id="numberRelateableProductsDiv{{ $loop->iteration}}"
                                    class="product-sizes-listing d-none value-button"
                                    data-size="{{ getProductAttr($related_product->id, 'size') }}"
                                    data-price="{{ $product_price }}"
                                    data-stock="{{ getProductAttr($related_product->id, 'stock') }}">
                                    {{ getProductAttr($related_product->id, 'size') }}
                                </div>
                                <div class="value-button" id="decrease"
                                    onclick="decreaseValue('numberRelateableProducts', {{ $loop->iteration }})"
                                    value="Decrease Value">
                                    -
                                </div>
                                <input type="number" id="numberRelateableProducts{{ $loop->iteration }}" value="1"
                                    class="product_qty" data-id="{{ $related_product->id }}"
                                    data-sp="{{ $product_price }}" data-title="{{ $related_product->product_name }}" />
                                <div class="value-button" id="increase"
                                    onclick="increaseValue('numberRelateableProducts', {{ $loop->iteration }})"
                                    value="Increase Value">
                                    +
                                </div>
                                <button class="Add-to-card value-button addToCartAjax"
                                    data-pcount="{{ $loop->iteration }}" data-section="numberRelateableProducts">Add to
                                    Cart</button>
                            </form>
                        </div>
                    </div>
